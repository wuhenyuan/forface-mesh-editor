/**
 * 布尔操作器
 * 使用 three-bvh-csg 库处理实体几何体之间的布尔运算。
 *
 * 这个类是通用底层能力，不再隶属于 surfaceText。
 * 文字、模型或其它实体都应该通过它完成底层 CSG 计算。
 */
  ...
  async subtract(
    ...
    try {
      ...
      const targetEntityKey =
        (typeof options.targetEntityKey === 'string' && options.targetEntityKey) ||
        (typeof options.sourceEntityId === 'string' && options.sourceEntityId) ||
        null;
      const toolEntityKey =
        (typeof options.toolEntityKey === 'string' && options.toolEntityKey) ||
        (typeof options.regionOwnerEntityId === 'string' && options.regionOwnerEntityId) ||
        (typeof options.entityKey === 'string' && options.entityKey) ||
        (typeof options.textId === 'string' && options.textId) ||
        null;
      ...
      annotateEntityMaterial(targetMaterial, {
        entityKey: targetEntityKey || undefined,
        sourceEntityId: targetEntityKey || undefined,
        regionRole: 'base',
      });
      annotateEntityMaterial(toolMaterial, {
        entityKey: toolEntityKey || undefined,
        sourceEntityId: toolEntityKey || undefined,
        regionOwnerEntityId: toolEntityKey || undefined,
        textId: typeof options.textId === 'string' ? options.textId : undefined,
        isEngravedText: true,
        regionRole: typeof options.regionRole === 'string' ? options.regionRole : 'subtract',
      });
