...
export class FeatureDetector {
  ...
  async _preprocessSingleMesh(mesh) {
    ...
    try {
      ...
      console.log(
        `特征预处理完成: ${features.planes.length} 个平面, ${features.cylinders.length} 个圆柱, 耗时: ${processingTime.toFixed(2)}ms`
      );
  ...
  async detectPlanes(triangles) {
    ...
    for (let i = 0; i < triangles.length; i++) {
      ...
      const plane: CoreValue = this.growPlane(triangles, i, processed);
  ...
  async detectCylinders(triangles) {
    ...
    for (let i = 0; i < triangles.length; i++) {
      ...
      const cylinder: CoreValue = this.growCylinder(triangles, i, processed);
  ...
  growCylinder(triangles, seedIndex, processed) {
    ...
    while (
      queue.length > 0 &&
      cylinder.triangleIndices.length < this.config.maxTrianglesPerFeature
    ) {
      const currentIndex = queue.shift();
