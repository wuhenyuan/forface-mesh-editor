...
export class VertexBasedIdentifier {
   ...
   * 为特征生成基于顶点索引的稳定标识
   */
  generateVertexBasedId(triangleIndices: number[], geometry: THREE.BufferGeometry): string {
   ...
   * 从三角形索引中提取所有顶点索引
   */
  extractVertexIndices(triangleIndices: number[], geometry: THREE.BufferGeometry): number[] {
   ...
   * 压缩连续的索引
   */
  compressConsecutiveIndices(indices: number[]): string {
   ...
   * 解压缩索引字符串（用于调试和验证）
   */
  decompressIndices(compressed: string): number[] {
  ...
  calculateCompressionStats(originalIndices: number[], compressed: string): {
    ...
    const originalSize = originalIndices.length * 4
   ...
   * 验证压缩和解压的一致性
   */
  validateCompression(originalIndices: number[]): boolean {
