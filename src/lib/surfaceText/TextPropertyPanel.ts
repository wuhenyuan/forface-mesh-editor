...
export class TextPropertyPanel {
  ...
  resetToDefault(propertyName) {
    if (propertyName) {
      ...
      // 重置所有属性
      Object.entries(this.properties).forEach(([name, config]) => {
        if (config.default !== undefined) {
          this.setPropertyValue(name, config.default)
        }
      })
    }
  }
  ...
  createPanelData() {
    ...
    const properties = Object.entries(this.properties).map(([name, config]) => ({
      name,
      ...config,
