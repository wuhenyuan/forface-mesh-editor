...
export class TextGeometryGenerator {
  ...
  loadedFonts: Map<string, any>
  defaultFont: any
  cylinderTextGenerator: CylinderTextGeometry
  csgCylinderText: CSGCylinderText
  cylinderTextMethod: string
  ...
  generateFlatText (text, font, config) {
    ...
    const geometryParams = {
      font: font,
      size: config.size,
      depth: config.thickness, // Three.js中使用depth表示厚度
   ...
   */
  createFallbackGeometry (text: string = '', config: Record<string, any> = {}) {
